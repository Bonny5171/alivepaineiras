...
export default function SocioCultural() {
  ...
  const [activeFilter, setActiveFilter] = useState<string>('CULTURAL');
  ...
  const fetchActivities = async () => {
    ...
    try {
      const atividadesFiltro = await listarAtividadesFiltro({ TITULO: selectedAssociated?.TITULO, IDAREA: 2 });

      const filterSports = atividadesFiltro.filter((item) => item.IDAREA === 2);

      const response = [...filterSports];
  ...
  const fetchActivityEnrolled = async () => {
    ...
    try {
      ...
      const [count] = await Promise.all([
        await listarQuantidade({ TITULO: selectedAssociated.TITULO, IDAREA: 2 })
      ])
  ...
  const filteredActivities = activities.filter((item) => {
    ...
    return item.IDDEPARTAMENTO === areaMap && matchesSearch;
  });
  ...
  return (
    <View style={[styles.container, { backgroundColor: background }]}>
      <Header title="Inicio" />
      <SectionTitle text='Socio-cultural' />
      <ScrollView contentContainerStyle={[styles.scrollContent, { marginTop: 16 }]}>
        ...
        <View style={[styles.statusCards, { paddingHorizontal: 16 }]}>
        ...
        <View style={{ paddingHorizontal: 16 }}>
          <View style={styles.filters}>
            {[
              ...
              { label: 'Cursos de Férias', value: 'FERIAS' }
            ].map((f, index) => (
              <TouchableOpacity
                key={String(f.value)}
              ...
              >
                <Text
                  style={[
                    ...
                    { color: tabText },
                    activeFilter === f.value && styles.activeFilterTabText,
                  ]}
                >
                  {f.label}
                </Text>
            ...
            {isLoadingActivities ? (
              ...
              filteredActivities.map((act) => {
                const DESCRICAO_CATEGORIA =
                  act.IDAREA === 2 && 'Cultural'
                return (
                  <TouchableOpacity
                    key={act.IDENTIFICADOR}
                    ...
                    onPress={() => {
                      ...
                      if (selectedAssociated && selectedAssociated.TITULO === '0') {
                        ...
                        const userAssociate = associated.find(a => a.TITULO === AuthContext.user);
                        if (userAssociate) {
                          associateToSend = userAssociate;
                        }
                      }
                      // Remove o item 'Todos' antes de passar para a próxima tela
                      const associatesToSend = associated.filter(a => a.TITULO !== '0');
                      const params = {
                        ...act,
                        ERRO: '',
                        ...
                        dependentesAssociates: associatesToSend // Sem o item 'Todos'
                      };

                      (navigate as any).navigate('(tabs)/(sports)/schedules', params);
                    }}
                  >
                    <View style={[styles.iconCircle, { backgroundColor: lightPinkColor }]}>
                      <IconSymbol name={act.ICONE} size={20} library='fontawesome' color={'#da1984'} />
                    </View>
                    <View style={styles.activityTextContainer}>
                      <Text style={[styles.activityTitle, { color: textColor }]}>{act.DESCRICAO}</Text>
                      <Text style={[styles.activitySubtitle, { color: '#878da3' }]}>Cultural</Text>
                    </View>
...
const ModalContent = (props: {
  ...
  const fetchEnrolledActitivites = async () => {
    ...
    try {
      ...
      const response = await request({ TITULO: selectedAssociated?.TITULO, IDAREA: 2 });

      const filteredResponse = response.filter((item) => item.IDAREA === 2);

      const formatted = filteredResponse.map((item) => ({
        ...item,
        ATIVIDADE: item.ATIVIDADE || item.GRUPO,
        STATUS: type
      }))
                  ...
                  {item.AVATAR ? (
                    ...
                    <View style={styles.initialsAvatar}>
                      <Text style={{ color: '#0F1C47', fontWeight: 'bold', fontSize: 18 }}>
                        {(() => {
                          // Prefer ASSOCIADO, fallback to NOME, then TITULO
                          const name = (typeof item.ASSOCIADO === 'string' && item.ASSOCIADO)
...
const styles = StyleSheet.create({
  ...
  statusCard: { flex: 1, backgroundColor: '#D034811A', paddingHorizontal: 8, paddingVertical: 16, borderRadius: 16, alignItems: 'center' },
