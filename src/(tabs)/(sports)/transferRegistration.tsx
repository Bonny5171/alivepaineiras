...
const TranferRegistrations = ({ route }: any) => {
  const { setError } = useError();
  const { IDENTIFICADOR, DESCRICAO, ICONE, IDAREA, ANTIGATURMA, USERAVATAR, USERTITULO } = route.params;
  const navigation = useNavigation();

  const [isLoading, setIsLoading] = useState(true);
  const [data, setData] = useState<AtividadesAgendaHorariosItem[]>([]);
  ...
  const fetchSchedules = async () => {
    ...
    try {
      const response = await listarTranferencias({
      ...
      >
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.subtitle}>Matrícula Obrigatória</Text>
          </View>
        </View>
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      navigation.navigate(
                        "(tabs)/(sports)/transferRegistrationDetails", {
                          ...item,
                          IDAREA,
                          DESCRICAO,
...
export default TranferRegistrations;